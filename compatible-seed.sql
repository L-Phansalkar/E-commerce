-- Compatible seed that works with existing schema
-- This will adapt to whatever tables already exist

-- First, let's check and create only missing tables
...
-- Try to add missing columns if they don't exist (these will fail silently if columns exist)
...
ALTER TABLE products ADD COLUMN songs TEXT;
ALTER TABLE products ADD COLUMN stripe TEXT;

-- Clear existing product data only (safer approach)
DELETE FROM products WHERE 1=1;

-- Seed Products (Billy Bass collection)
...
-- Only seed users if the table is compatible
